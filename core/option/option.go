...
const (
	...
	// tcpKeepAliveEnabled is the value used to enable or disable keepalives
	// on the socket.
	tcpKeepAliveEnabled = true

	// tcpDefaultKeepaliveCount is the maximum number of TCP keep-alive
	// probes to send before giving up and killing the connection if no
	// response is obtained from the other end.
	tcpDefaultKeepaliveCount = 9

	// tcpDefaultKeepaliveIdle specifies the time a connection must remain
	// idle before the first TCP keepalive packet is sent.
	tcpDefaultKeepaliveIdle = 60 * time.Second

	// tcpDefaultKeepaliveInterval specifies the interval time between
...
// TCPSocketOption applies a TCP socket-level option to a single endpoint.
...
// WithKeepAliveEnabled enables or disables TCP keepalive on the endpoint.
...
// WithTCPKeepaliveInterval sets the TCP keepalive probe interval.
...
// WithTCPKeepaliveCount sets the TCP keepalive probe count.
