...
func netstack(k *Key) (err error) {
	...
	if k.TCPKeepaliveIdleTime > 0 {
		...
		log.Infof("[STACK] TCP keepalive idle time: %v", k.TCPKeepaliveIdleTime)
	} else {
	...
	if k.TCPKeepaliveInterval > 0 {
		...
		log.Infof("[STACK] TCP keepalive interval: %v", k.TCPKeepaliveInterval)
	} else {
	...
	if k.TCPKeepaliveCount > 0 {
		...
		log.Infof("[STACK] TCP keepalive count: %d", k.TCPKeepaliveCount)
	} else {
